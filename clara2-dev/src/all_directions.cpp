/**
 * Copyright 2014-2018 Richard Pausch, Joy, Alexander Koehler
 *
...
int all_directions(const unsigned int trace_id)
{
  ...
  {
    theta[i] = 2.0 * (double)i / (param::N_theta -1) * param::theta_max - param::theta_max;
    //theta[i] = (double)i / param::N_theta * param::theta_max;
  }
  std::cout << "THETA RANGE:" << theta[0] << " , " << theta[param::N_theta - 1] << std::endl;
  ...
  double delta = (log10(param::omega_max) - log10(param::omega_min))/param::N_spectrum;
  std::cout << "w_max: " << param::omega_max << std::endl;
  std::cout << "w_min: " << param::omega_min << std::endl;
  std::cout << "lgmax: " << log10(param::omega_max) << std::endl;
  std::cout << "lgmin: " << log10(param::omega_min) << std::endl;
  std::cout << "delta: " << delta << std::endl;

  for(unsigned i=1; i<param::N_spectrum; ++i)
  {
    /* compute frequency */
    omega[i] = (i * my_delta_omega);
    //omega[i] = pow(10, i*delta) * param::omega_min;
  ...
  {
    std::cout<< "Computing: " << direction_index << " of " << N_direction << std::endl;
    const double my_theta = theta[direction_index % param::N_theta];
    const double my_phi   = phi[direction_index/param::N_theta];
    std::cout << "calculating theta: " << my_theta << " phi: " << my_phi << std::endl;
